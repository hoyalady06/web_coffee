...
export default function ManagerOrderDetailsPage() {
  ...
  async function load(orderId: string) {
    ...
    const res = await fetch(`/api/manager/order/${orderId}`);
  ...
  async function changeStatus(newStatus: string) {
    ...
    if (data.ok) {
      await load(order.id); // 🔄 полностью как у админа
    } else {
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border p-5 space-y-3">
        ...
        {order.delivery_type === "delivery" && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-sm text-gray-800 flex gap-3">
                <span className="text-yellow-500 mt-0.5">⚠️</span>
                <div>
                <p className="font-medium">
                    Доставка производится в двух промежутках:
                </p>
                <p className="mt-1">
      ...
      <div className="bg-white rounded-xl border p-5 space-y-2">
        ...
        <p>Дата: <b>{order.delivery_date || "—"}</b></p>
        <p>Время: <b>{order.delivery_time || "—"}</b></p>
      </div>
